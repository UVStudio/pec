...
//@desc   Create or update a current user's profile
//@access Private
...
router.put(
  ...
  async (req, res) => {
    ...
    const exp = { title, company, startDate };
  ...
  [
    ...
    [
      ...
      check('fieldofstudy', 'Field of Study date is required.')
        .not()
        .isEmpty(),
      check('startDate', 'Start Date date is required.')
        .not()
        .isEmpty()
    ]
  ],
...
router.put(
  ...
  async (req, res) => {
    ...
    // since title, company, from are required
    const edu = { school, degree, fieldofstudy, startDate };
    if (to) edu.to = to;
...
//@desc   Getuser repos from github
//@access Public
